...
execute at @e[tag=dd] run effect give @e[distance=..100,tag=!dd] minecraft:glowing 1 1 true
...
## put on mask
execute as @e[scores={blackmask=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{display:{Name:"{\"text\":\"The Black Mask\",\"color\":\"black\",\"bold\":\"true\",\"italic\":\"true\"}"},HideFlags:63,Unbreakable:1b,Damage:2b}}}] run tag @s add putonmask
scoreboard players reset @e blackmask
execute as @e[tag=putonmask] unless entity @s[nbt={Inventory:[{Slot:103b}]}] run replaceitem entity @s weapon.mainhand air
execute as @e[tag=putonmask] unless entity @s[nbt={Inventory:[{Slot:103b}]}] run replaceitem entity @s armor.head minecraft:carrot_on_a_stick{display:{Name:"{\"text\":\"The Black Mask\",\"color\":\"black\",\"bold\":\"true\",\"italic\":\"true\"}"},HideFlags:63,Unbreakable:1b,Damage:2b} 1
